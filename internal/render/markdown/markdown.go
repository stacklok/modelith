...
var (
	// qualifiedReferenceRE matches a reference to an imported item,
	// "scope.Name". It mirrors the linter's pattern; the renderer only decides
	// whether to link, never whether the reference resolves.
	qualifiedReferenceRE = regexp.MustCompile(`^(` + model.ScopeSlug + `)\.([A-Z][A-Za-z0-9]*)$`)
...
// qualifiedReference renders a qualified entity or type as a deep link into the
// imported model's Markdown. A scope no import binds — a lint error — renders as
// written rather than as a link to nowhere.
func qualifiedReference(ref string, targets map[string]string) string {
	match := qualifiedReferenceRE.FindStringSubmatch(ref)
	if match == nil {
		return codeSpan(ref)
	}
	...
	return fmt.Sprintf("[%s](%s#%s)", ref, linkTarget(target), strings.ToLower(match[2]))
}

// typeCell renders an attribute type for a table cell, linking a "scope.Name"
// reference into the imported model's Markdown.
func typeCell(typ string, targets map[string]string) string {
	match := qualifiedReferenceRE.FindStringSubmatch(typ)
	if match == nil {
		return mdCell(typ)
	}
	target, ok := targets[match[1]]
...
func renderEntities(b *strings.Builder, m *model.Model, sourceDir, outDir string) {
	...
	for _, name := range names {
		...
		if ent.SubtypeOf != "" {
			fmt.Fprintf(b, "**Subtype of** %s\n\n", qualifiedReference(ent.SubtypeOf, importTargets))
		}
		...
		if len(ent.Relationships) > 0 {
			...
			for _, rel := range ent.Relationships {
				parts := []string{qualifiedReference(rel.Entity, importTargets), rel.Cardinality}
				if rel.Symmetric {
